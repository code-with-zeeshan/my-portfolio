...
import { mapStaticExperience } from "@/lib/utils";
...
export default function DynamicExperience() {
  ...
  const displayExperiences = supabaseDown
    ? staticExperiences.map((e, i) => mapStaticExperience(e, i))
    : experiences ?? [];
              ...
              {displayExperiences.map((exp, idx) => (
                <FadeIn key={exp.id} delay={idx * 0.15}>
                  <div className="relative">
                    ...
                    <div
                      className={`pl-10 md:pl-0 md:grid md:grid-cols-2 md:gap-12`}
                    >
                      {idx % 2 === 0 ? (
                        <>
          ...
          <FadeIn>
            <div className="rounded-2xl border border-dashed border-zinc-300 p-16 text-center dark:border-zinc-700">
              ...
              <p className="text-sm text-zinc-500 max-w-xs mx-auto">
                Career history and achievements will appear here once added from
                the admin dashboard.
              </p>
            </div>
          </FadeIn>
