// src/components/react/BlogTab.tsx
"use client";
...
export default function BlogTab({
  ...
  const updateBlogField = (id: string, field: string, value: any) => {
    setBlogPosts((prev) =>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      const { error } = await supabase
                        .from("blog_posts")
                        .update({
                          description: editingBlogPost.description,
                        })
                ...
                <div className="flex items-center gap-3">
                  ...
                  {post.pub_date && (
                    <span className="text-xs text-zinc-400">
                      {new Date(post.pub_date).toLocaleString(undefined, {
                        dateStyle: "medium",
                        timeStyle: "short",
                      })}
                    </span>
              ...
              <div className="p-6 space-y-5">
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <Field label="Title">
                    <input
                      ...
                      value={post.title}
                      onChange={(e) =>
                        updateBlogField(post.id, "title", e.target.value)
                      }
                      className={inputCls}
                    />
                  </Field>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    onChange={(e) =>
                      updateBlogField(post.id, "description", e.target.value)
                    }
                    className={inputCls}
                  />
                </div>
                ...
                <div className="grid gap-3 md:grid-cols-2">
                  <Field label="Tags">
                    <TagInput
                      ...
                      onChange={(newTags: string[]) =>
                        updateBlogField(post.id, "tags", newTags)
                      }
                  ...
                  <Field label="Hero Image">
                    <CloudinaryUpload
                      ...
                      onUpload={(result) =>
                        updateBlogField(post.id, "hero_image", result.url)
                      }
                      onRemove={() =>
                        updateBlogField(post.id, "hero_image", null)
                      }
                    />
                  </Field>
                </div>
                ...
                <Field label="Content (Markdown)">
                  <MarkdownEditor
                    value={post.content || ""}
                    onChange={(val) =>
                      updateBlogField(post.id, "content", val)
                    }
                    height={450}
                ...
                <div className="rounded-xl border border-zinc-200 bg-zinc-50 p-5 space-y-4 dark:border-zinc-700 dark:bg-zinc-800/50">
                  ...
                  <Field label="OG Image URL (1200×630px — for social sharing)">
                    <CloudinaryUpload
                      ...
                      onUpload={(result) =>
                        updateBlogField(post.id, "og_image", result.url)
                      }
                      onRemove={() =>
                        updateBlogField(post.id, "og_image", null)
                      }
                    />
                  </Field>
                    ...
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        if (error) {
                          ...
                          notify(
                            "error",
                            `Failed to ${publishing ? "publish" : "unpublish"}: ${error.message}`
                          );
                        } else {
                          notify(
                            "success",
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        {post.scheduled_for && (
                          <button
                            type="button"
                            onClick={() =>
                              updateBlogField(post.id, "scheduled_for", null)
                            }
